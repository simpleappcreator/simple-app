new ngDirective('durationPicker', function durationPicker($scope, element, attrs) {
    ...
    Object.defineProperty($scope, 'disabled', { 
        get: function get() {// because attrs.disabled comes and goes
    ...
    function input_changed() {
        ...
        switch ($scope.input.unit) {
            ...
            case 'hours':
                if ($scope.input.value <= 0) {
                    $scope.input.value = 59;
                    $scope.input.unit = 'minutes';} else 
                if ($scope.input.value >= 24) {
                    $scope.input.value = 1;
            ...
            case 'days':
                if ($scope.input.value <= 0) {
                    ...
                    $scope.input.unit = 'hours';} else 
                if ($scope.input.value >= 8) {
                    $scope.input.value = 1;
            ...
            case 'forever':
                ...
                if ($scope.input.value <= 0) {
                    $scope.input.value = 7;
                    $scope.input.unit = 'days';
                    $scope.forever = $scope.$parent.forever = false;} else 
                if ($scope.input.value >= 1) {
                    $scope.input.value = 1;}
    ...
    if (typeof $scope.output[$scope.name] != 'undefined') output_changed();else 
    if (typeof attrs.default != 'undefined' && attrs.default.length) {
        if (attrs.default == 'forever') 
        output_changed($scope.output[$scope.name] = $scope.max);else 
        if (match = attrs.default.match(/([0-9]+)([\w]*)s?/)) {
            $scope.input = { 
