...
$sce) 
{
    ...
    $rootScope.trustAsHtml = $sce.trustAsHtml.bind($sce);
    ...
    $rootScope.Error = function ShowErrorMessage(err) {
        ...
        if (err) {
            ...
            this.err = this.error = err;else 
            if (err.message) 
            this.err = this.error = err.message;else 
            if (err.status) 
            this.err = this.error = err.status + ' ' + err.statusText + ': ' + err.data.substr(0, 100);} else 
