...
mongoose.url = config.mongodburl;else 
...
process.env.OPENSHIFT_APP_NAME;else 
if (process.env.C9_USER) 
mongoose.url = 'mongodb://' + process.env.IP + '/' + database;else 
if (process.env.MONGOLAB_URI) 
mongoose.url = process.env.MONGOLAB_URI;else 
if (process.env.MONGOHQ_URL) 
mongoose.url = process.env.MONGOHQ_URL;else 

mongoose.url = 'mongodb://localhost/' + database;

mongoose.connect(mongoose.url, function (err, res) {
    if (err) console.error('Mongoose couldn\'t connect to:', mongoose.url, err);else 
    console.verbose('Mongoose connected to:', mongoose.url);});
//.set('debug', true);
...
mongoose.model = function mongoose_model_patch(collectionName, schema) {for (var _len = arguments.length, restArgs = Array(_len > 2 ? _len - 2 : 0), _key = 2; _key < _len; _key++) {restArgs[_key - 2] = arguments[_key];}var _mongoose;
    ...
    var model = (_mongoose = mongoose)._model.apply(_mongoose, [collectionName, schema].concat(restArgs));
