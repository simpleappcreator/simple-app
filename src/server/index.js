...
global.connectRestreamer = require('connect-restreamer');
...
// Middlewares
global.locals = app.locals.locals = app.locals;
...
function repl() {
    rl.question('', function(command) {
        if (command.match(/^(q|e|r)$/)) process.exit(0);
        else repl();
    });
}
if (dev || config.repl) repl();
