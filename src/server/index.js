...
app.use(function (req, res, next) {
    ...
    res.redirect = function res_redirect_patch(url, err) {
        if (err) 
        req.session.err = err.message || err + '';else 
...
function defaultThirdPartyAuthRoute(req, res, next) {
    ...
    req.session.googleAuthFlag = true;else 
    if (!req.session.googleAuthFlag) 
    console.debug('how dare you?');else 
    {
        console.debug('cool!');
...
function catchErrors(err, req, res, next) {
    ...
    if (req.json) res.status(status).json(err.message);else 
