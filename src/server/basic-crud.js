function _defineProperty(obj, key, value) {if (key in obj) {Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true });} else {obj[key] = value;}return obj;}function _typeof(obj) {return obj && typeof Symbol !== "undefined" && obj.constructor === Symbol ? "symbol" : typeof obj;}function _classCallCheck(instance, Constructor) {if (!(instance instanceof Constructor)) {throw new TypeError("Cannot call a class as a function");}} // Defaults
var bc = locals.bc = { 
    'ngTemplateMultipleFull': null, 
    'ngTemplateMultipleForm': null, 
    'ngTemplateSingleFull': null, 
    'ngTemplateSingleForm': null };var 


BasicCrud = 
function BasicCrud(args, args2, args3) {_classCallCheck(this, BasicCrud);
    var bc = this;
    ...
    for (var key in args) {
        bc[key] = args[key];}
    if ((typeof args2 === 'undefined' ? 'undefined' : _typeof(args2)) == 'object') 
    ...
    if ((typeof args3 === 'undefined' ? 'undefined' : _typeof(args3)) == 'object') 
    ...
    {
        try {
            bc.schema = bc.db.schema.tree;
            delete bc.schema._id;
            delete bc.schema.id;
            delete bc.schema.hash;
            delete bc.schema.__v;} 
        catch (err) {}}
...
BasicCrud.prototype.addDatabase = function () {
    var bc = this;
    if (!bc.schema) bc.schema = _defineProperty({}, 
    bc.camelCase, 'String');
...
BasicCrud.prototype.addRoute_PRElocals = function (path) {
    ...
    if (path) app.get(path, route);else 
    app.get('/' + (bc.admin ? 'admin/' : '') + bc.slug + '*', route);
    return bc;
...
BasicCrud.prototype.addRoute_PRErefs = function (path) {
    ...
    function route(req, res, next) {
        ...
        locals.refs = {};
        var limit = Infinity;
        Promise.all(Object.keys(bc.schema).reduce(function (refs, key) {
            // Collect unique refs
        ...
        []).map(function (ref) {
            ...
            then(function (docs) {return locals.refs[ref] = docs;}).
            catch(console.err);})).
        then(function (d) {return next();}).catch(next);}};


BasicCrud.prototype.addRoute_GETmultiple = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.get('/' + (bc.admin ? 'admin/' : '') + bc.slugs, route);
    return bc;

    function route(req, res, next) {
        var locals = res.locals;
        var Model = res.mongoose && res.mongoose.model(bc.collectionName) || bc.db;
        (Model.findAllAsync ? Model.findAllAsync(req) : Model.find({}).lean().limit(bc.limit || Infinity).sort(bc.sortOrder || '').execAsync()).
        then(function (docs) {
            locals.title = bc.Names + ' | ' + config.Name;
            locals.data = locals[bc.names] = docs;
            if (req.json) res.json(locals.data);else 
            res.render(bc.viewMultiple || 'basic-crud/multiple');}).
        catch(next);}};
...
BasicCrud.prototype.addRoute_GETsingle = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.get('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/:title/:id', route);
    return bc;
    ...
    function route(req, res, next) {
        ...
        findByIdAsync(req.params.id).then(function (doc) {
            ...
            if (req.json || req.url.match(/json$/)) res.json(doc);else 
            res.render(bc.viewSingle || 'basic-crud/single');}).
        catch(function (err) {
...
BasicCrud.prototype.addRoute_GETadd = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.get('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/add', route);
    return bc;
...
BasicCrud.prototype.addRoute_POSTupdate = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.post('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/update', route);
    return bc;
    ...
    function route(req, res, next) {
        ...
        Model.findByIdAndUpdate(id, data).execAsync().then(function (doc) {
            if (password && password.length && doc.savePassword) {
                ...
                return doc.saveAsync();} else 
            return doc;}).
        then(function (doc) {
            console.verbose('%s Update request fulfilled\n', bc.Name, doc);
            if (req.json) res.json(doc);else 
            res.redirect('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/' + getDocSlug(doc) + '/' + doc.id);}).
        catch(function (err) {
...
BasicCrud.prototype.addRoute_POSTremove = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.post('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/remove', route);
    return bc;
    ...
    function route(req, res, next) {
        ...
        Model.findById(id).removeAsync().then(function () {
            ...
            if (req.json) res.json('removed');else 
            res.redirect('/' + (bc.admin ? 'admin/' : '') + bc.slugs);}).
        catch(function (err) {
...
BasicCrud.prototype.addRoute_POSTadd = function (path) {
    var bc = this;
    if (path) app.get(path, route);else 
    app.post('/' + (bc.admin ? 'admin/' : '') + bc.slug + '/add', route);
    return bc;
...
//# sourceMappingURL=basic-crud.js.map
